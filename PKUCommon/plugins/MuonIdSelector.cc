...
//        reco::VertexRef vtx(vtxs, 0);}
    ...
    double pt = mu.pt();
    double eta = mu.eta();
    isTight = mu.isHighPtMuon( *firstGoodVertex ) && (pt > 50) && (fabs(eta) < 2.1) && (trackIso/pt<0.1);//&& (trackIso/pt<0.1);// && (abs(eta)<2.4);//->position());
    isMedium = mu.isHighPtMuon( *firstGoodVertex );
    isLoose = mu.isHighPtMuon( *firstGoodVertex ) && (pt > 20) && (fabs(eta) < 2.4) && (trackIso/pt<0.1);
    // ---------- cut-based ID -----------------

    isPassPteta = (pt > 40) && (fabs(eta) < 2.4) && (trackIso/pt<0.1);
